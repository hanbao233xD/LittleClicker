...
## 2026-06-18（运行一次模式支持运行次数）
- 需求实现：
  - 在“参数设置 -> 运行方式”中，当选择 `运行一次` 时，显示 `运行次数` 输入框；
  - 默认值为 `1`；
  - 点击开始后，会将整套脚本重复执行，直到达到设定次数。
- 代码改动：
  - `ActionManageScreen`：在 `RunOnce` 模式下新增 `运行次数` 数字输入框；
  - `AutoClickAccessibilityService.executeProfile(...)`：恢复 `cycleCount` 在 `RunOnce` 模式下的执行次数语义；
  - `AutoClickCoordinator.startNow(...)`：运行状态文案补充当前设定次数。
...
- 文档更新：
  - 重写 `readme.md`，在开头加入项目宣传图，并补充项目定位、核心功能、界面预览、安装使用、权限说明、构建方式和项目结构。
  - 新增 `docs/img/readme_hero.png`，基于当前真实 UI 截图合成 README 头图；手机界面 mockup 已加入安卓原生状态栏与居中挖孔。
- 验证结果：
  - 本轮仅修改文档和图片资源，未执行 Android 编译。
...
## 2026-04-26（运行方式与循环延迟迁移到配置管理 + 配置管理入口并入点击配置）
- 需求实现：
  - 将“运行方式”与“每次循环延迟(ms)”编辑能力从首页移入“配置管理”页面；
  - 将首页“配置管理”独立卡片移动到“点击配置”分区内，作为子入口显示。
- 修复内容：
  - `AutoClickScreen`：
    - 移除“点击配置”卡片中的“运行方式”下拉与“每次循环延迟(ms)”输入框；
    - 在“点击配置”卡片内新增“配置管理”子区块（显示当前配置名/循环次数 + “打开配置管理”按钮）；
    - 删除原有独立“配置管理”卡片，完成分区归位。
  - `ConfigManageScreen`：
    - 在“当前配置编辑”中新增“运行方式”下拉；
    - 在循环模式下显示“每次循环延迟(ms)”输入框（仅数字）并更新 `loopIntervalDelayMs`；
