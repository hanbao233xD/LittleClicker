...
1. 已完成阶段：`P1`（项目初始化与底部导航架构）+ `P2` 自动点击优先重构（自动点击页、自动点击悬浮窗、保存能力）。
2. 默认起始阶段：`P3`（若明确要求可回到任意阶段回归）。
3. 基线依据：`docs/IMPLEMENTATION_LOG.md` 中 `2026-03-23` 记录。
4. 基线约束：后续实现不得破坏 `自动点击/脚本管理/关于` 三 Tab 和已有权限引导流程。

## A2. 阶段重排补充（2026-03-23）
若与下方旧版 `P2~P5` 描述冲突，以本补充为准。

1. 已提前完成并并入当前基线：
   - 自动点击主界面（默认首页）与参数配置（每点延迟/触摸时长/重复次数、全局循环）。
   - 自动点击专用悬浮窗（可拖动面板 + 点位拖动 + 添加点位 + 运行控制 + 保存）。
   - 自动点击执行引擎（`dispatchGesture` 基础点击、开始/暂停/继续/停止）。
   - 自动点击配置保存与脚本草稿保存（App 私有 JSON）。
   - 应用内可靠定时开始（进程存活时触发）。
2. 明确后移到后续阶段（避免抢跑）：
   - 脚本录制与动作采集（原 `P4`）。
   - 脚本动作编辑、脚本执行、脚本专用悬浮窗（后续脚本阶段）。
   - 系统级后台唤醒定时（`AlarmManager` 精确定时与兜底）。
3. 后续阶段建议顺序：
   - `P3`：补齐 click/swipe 扩展能力与紧急停止策略（在已落地点击引擎上增强）。
   - `P4`：脚本录制与动作模型扩展（从草稿结构升级到可执行脚本）。
   - `P5`：脚本管理深度功能（编辑、参数配置、脚本执行接入）。
   - `P6+`：系统级定时、随机扰动、防检测与高阶生态功能。
